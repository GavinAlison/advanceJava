package com.alison.command.string;
...
public class StringCommand extends BaseConnection {
    ...
    /**
     * 将存储的数字减key1。如果密钥不存在，则0在执行操作之前将其设置为。如果密钥包含错误类型的值或包含无法表示为整数的字符串，则会返回错误。
     * 该操作仅限于64位有符号整数。没有对应的key则设置为0，再相减
     * DECR key
     * 返回值：key减量后的值
     * 命令：
     * set decrKey 6
