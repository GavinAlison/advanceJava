...
public class GenericCommand extends BaseConnection {
     ...
     * KEYS 我试了没效果，望指点。官网原话：If the key argument is an empty string, the command will instead migrate all the keys that follow the KEYS option
     * (see the above section for more info)
    ...
    public void migrate() {
        ...
        jedis.migrate("192.168.56.103", 6379, "migrate", 0, 5000);
        Jedis otherJedis = new Jedis("192.168.56.103");
        System.out.println(otherJedis.get("migrate"));

        //spring
        RedisNode.RedisNodeBuilder redisNodeBuilder = new RedisNode.RedisNodeBuilder();
        redisNodeBuilder.listeningAt("192.168.56.103", 6379);
